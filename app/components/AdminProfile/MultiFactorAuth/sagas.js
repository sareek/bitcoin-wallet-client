...
function* disableMultiFactorAuthFlow(action) {
  ...
  yield fork(API.put(`multi-factor-auth/totp-disable/${action.payload}`, disableMultiFactorAuthSuccess,
...
function* loadBasicInfoFlow(payload) {
  const token = getToken();
  yield call(API.get(`user/data/${payload.userId}`, loadBasicInfoSuccess, loadBasicInfoFailure, token));
}
